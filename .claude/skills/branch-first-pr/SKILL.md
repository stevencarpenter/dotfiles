...
git add -A
git commit -m "type: imperative summary"      # one line; NO Co-Authored-By (see note)
# Append an issue reference ONLY when a number exists, as the whole token:
#   git commit -m "type: imperative summary (#42)"
# Substituting the summary but leaving a bare `)` behind is a recurring bug;
# five commits on refactor/uv-scripts ended in an unmatched `)` from this line.
...
One plaintext line, Conventional prefix, an optional `(#42)` issue reference kept whole or omitted entirely (never a
stray `)`), no emoji/markdown, and **never**
