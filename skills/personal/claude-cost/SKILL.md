...
"${CLAUDE_SKILL_DIR}/scripts/cost.py" week            # current ISO week
"${CLAUDE_SKILL_DIR}/scripts/cost.py" year            # year to date
```

Pick a provider, an explicit period, a trailing window, or JSON:

```bash
"${CLAUDE_SKILL_DIR}/scripts/cost.py" month --provider claude    # Claude Code only
"${CLAUDE_SKILL_DIR}/scripts/cost.py" month --provider codex     # Codex only
"${CLAUDE_SKILL_DIR}/scripts/cost.py" month 2026-04              # a specific month
"${CLAUDE_SKILL_DIR}/scripts/cost.py" week 2026-W20             # a specific ISO week
"${CLAUDE_SKILL_DIR}/scripts/cost.py" month --trailing          # rolling last 30 days
"${CLAUDE_SKILL_DIR}/scripts/cost.py" year --json               # JSON for piping
```
...
```bash
"${CLAUDE_SKILL_DIR}/scripts/cost.py" month --rates my-rates.json
```
