...
Yank `edge.internal` into register `h` and `8443` into register `p`. Delete the entire seed block,
including its heading and blank line. Fill both targets from those named registers.

Practice: `"hyiw`, `"pyiw`, `"hp`, and Insert-mode `<C-r>h`. Your bare `d` discards into `"_`, so
deleting the seed block will not disturb the named registers.
