...
Yank the canonical value `30` once. Replace every timeout value with that yank, then remove the
canonical line and its comment.

Practice: `yiw` writes the yank to register `0` and, through `unnamedplus`, the pasteboard. Use
`ciw<C-r>0` or a visual put for each replacement. Your black-hole `c` mapping prevents the old
timeouts from replacing the source value.
