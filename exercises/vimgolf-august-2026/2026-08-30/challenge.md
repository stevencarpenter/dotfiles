...
Practice: `qa` starts recording into register `a`, `q` stops, `@a` replays, and `@@` repeats the
last macro. Build a macro that edits one whole worker line and lands on the next line. Replay it
with a count.
