...
| New shells see old environment values | The tmux server and existing panes captured older environment | Inspect `tmux show-environment -g`; restart the affected shell or server deliberately. `update-environment` does not rewrite existing process environments. |
